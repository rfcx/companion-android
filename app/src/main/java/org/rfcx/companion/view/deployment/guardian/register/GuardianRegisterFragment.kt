...
class GuardianRegisterFragment : Fragment() {
    ...
    private fun registerGuardianWithInternet() {
        ...
        if (guid != null && userToken != null) {
            ApiManager.getInstance().getDeviceApi2(isProductionSelected(), requireContext()).registerGuardian("Bearer $userToken", GuardianRegisterRequest(guid, null, null)).enqueue(
