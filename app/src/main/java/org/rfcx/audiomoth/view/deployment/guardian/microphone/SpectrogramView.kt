...
class FrequencyView : View {
    ...
    public override fun onDraw(canvas: Canvas) {
        ...
        paint.color = Color.BLACK
        canvas.drawRect(rWidth + wColor.toFloat(), 0f, _width.toFloat(), _height.toFloat(), paint)
        ...
        if (logFrequency) {
            for (i in 1..4) {
                ...
                canvas.drawText("1e$i", rWidth + wColor.toFloat(), (1f - y) * _height, paint)
            }
        } else {
            var i = 0
            while (i < (samplingRate - 500) / 2) {
                canvas.drawText(
                    ...
                    _height * (1f - i.toFloat() / (samplingRate / 2)),
                    paint
                )
                i += 1000
            }
    ...
    private fun getRelativePosition(
        ...
        return if (log) Math.log10(1 + value - minValue.toDouble()).toFloat() / Math.log10(
